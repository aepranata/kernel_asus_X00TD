...
/*
 * [4.19 backport] linux/string_choices.h doesn't exist in this kernel
 * (added upstream much later than 4.19). The only function from it that
 * this file actually used is str_high_low(), inlined as a plain ternary
 * at its single call site below instead of adding a shim header.
 */
...
static int __damon_stop(struct damon_ctx *ctx)
{
	...
	if (tsk) {
		...
		mutex_unlock(&ctx->kdamond_lock);
		/*
		 * [4.19 backport] kthread_stop_put() doesn't exist in this
		 * kernel's linux/kthread.h (newer mainline helper). Inlined
		 * manually: it's just kthread_stop() + put_task_struct(),
		 * matching the get_task_struct() taken just above.
		 */
		kthread_stop(tsk);
		put_task_struct(tsk);
		return 0;
				  ...
				  unsigned long sz_limit)
{
	...
	do {
		...
		/*
		 * [4.19 backport] max(1, ...) triggers -Wcompare-distinct-
		 * pointer-types here because this kernel's max() macro does
		 * a strict typecheck and 'threshold' is unsigned int while
		 * the literal 1 is a plain int. Use 1U to match.
		 */
					...
					unsigned long *metric_value)
{
	...
	case DAMOS_WMARK_FREE_MEM_RATE:
		/*
		 * [4.19 backport] totalram_pages is an extern unsigned long
		 * variable in this kernel's mm.h, not a function (the
		 * totalram_pages() accessor function was introduced later
		 * in mainline). Drop the call parens accordingly.
		 */
...
{
	/*
	 * [4.19 backport] USLEEP_RANGE_UPPER_BOUND and usleep_range_idle()
	 * are newer mainline additions not present in this kernel's
	 * linux/delay.h. USLEEP_RANGE_UPPER_BOUND is derived from
	 * TICK_NSEC/max_slack_shift (not a fixed constant), so rather than
	 * guess a threshold, this just always uses usleep_range() -
	 * matching how kdamond_usleep() worked in DAMON before this
	 * long-sleep optimization was added upstream. Functionally
	 * equivalent, just without the extra efficiency tweak for very
