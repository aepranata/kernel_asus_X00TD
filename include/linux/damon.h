...
/*
 * [4.19 backport] get_random_u32_below() doesn't exist in this kernel's
 * linux/random.h (it's a newer mainline addition; this tree only has the
 * get_random_*_wait() family). prandom_u32() has existed since long before
 * 4.19 and is sufficient here since this value is only used for sampling
 * jitter, not anything security-sensitive.
 */
static inline u32 damon_random_u32_below(u32 ceil)
{
...
{
	return l + damon_random_u32_below(r - l);
}
